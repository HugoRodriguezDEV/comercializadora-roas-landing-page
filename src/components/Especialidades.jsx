      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {CATEGORIAS.map(({ Icon, tag, title, description, color, tagColor, iconColor }) => (
            <div
              key={title}
              className={`${color} rounded-2xl p-6 flex flex-col gap-4 group hover:scale-[1.02] transition-transform duration-300 cursor-pointer`}
            >
              <div className="flex items-start justify-between">
                <Icon className={`w-8 h-8 ${iconColor}`} />
                <span className={`${tagColor} text-xs font-sans font-semibold px-3 py-1 rounded-full`}>
                  {tag}
                </span>
              </div>
              <div>
